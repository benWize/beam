...
def get_last_run_id(workflow_id, repo_url, release_branch, release_commit,
                    github_token):
    """
  ...
  """
    ...
    print(url, github_token)
    data = request_url(
        url,
        github_token,
        params={
            "event": "pull_request",
            "branch": release_branch  #event: "push"
        },
    )
...
def filter_artifacts(artifacts, is_rc_version):
    # TODO: Filter non rc artifacts
    source_zip = "source_zip_rc" if is_rc_version else "source_zip"
    wheelhouse = "wheelhouse-rc" if is_rc_version else "wheelhouse"
    return [
        a for a in artifacts if (a["name"].startswith(source_zip)
...
if __name__ == "__main__":
    ...
    try:
        ...
        run_id = get_last_run_id(workflow_id, repo_url, release_branch,
                                 release_commit, github_token)
