...
"""Script for downloading GitHub Actions artifacts from 'Build python wheels' workflow.
   This script is based on the script in release/src/main/scripts/download_github_actions_artifacts.py,
   but it has been modified, so that it can run in a GHA workflow"""
import argparse
import itertools
import os
import pprint
...
def parse_arguments():
    """
    Gets all neccessary data from the user by parsing arguments or using env vars.
    ...
    if 'GITHUB_TOKEN' in os.environ:
        ...
        print("You need to set GITHUB_TOKEN as env var")
        sys.exit(1)
